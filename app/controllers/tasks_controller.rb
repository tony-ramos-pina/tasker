class TasksController < ApplicationController
  ...
  before_action :set_task, only: %i[ show edit update destroy setAsDone setAsNotDone ] 
  ...
  def setAsDone
    respond_to do |format|
      if @task.update(isDone: true, lastEditDate: DateTime.now)
  ...
  def setAsNotDone
    respond_to do |format|
      if @task.update(isDone: false, lastEditDate: DateTime.now)
        format.html { redirect_to tasks_path, notice: "Task was successfully set as not done." }
        #format.json { render :index, status: :ok, location: @task }
        format.js
      else
        format.html { render :edit, status: :unprocessable_entity }
